...
if (save_user) {
    var baseFile = Tool.getSaveFile("Filename/Directoty", "C:/Users/LukasC/Documents/Campbell/Dilution Experiment PT2/Set6")//Tool.getText("Filename/directory")
...
//# of times to repeat the 4 pulse set measurement
ntrials = 50
//dt between trials
trial_delay = 1
//Max led voltage
...
function pulseAndCapture(save,q,t) {
    Scope1.single();
    wait(.1);
    Wavegen.run();
    Scope1.wait();
    if (save) {
        ...
        if (q == 1) { //Sample
            ...
            nameArray = baseFile.split("/")
            nameArray = nameArray[nameArray.length-1].split("_")
            nameArray[0] = nameArray[0]+"-"+t
            samplename = nameArray.join("_")
            Scope1.Export(savepath+"/"+samplename+"_"+i.toString()+".csv", szView = "", fComments = true, fHeader = true, fLabel = true, fHeaderAsComment = false, szNotes = "")
        }
        if (q == 2) { //Post
...
if (Tool.question("start pre air")) {
    Wavegen.Channel1.Mode.text = "Custom";
    ...
    const questions = ["pre", "sample", "post"]
            ...
            for (var i = 1; i <= repeat; i++) {
                ...
                Wavegen.Channel1.Mode.text = "Custom";
                Wavegen.Channel1.Custom.Type.text = "ramp" + i;
                pulseAndCapture(save_user,0,trial);
                wait(parseInt(delay));
...
// Check if user wants space bt ramps to increase each iteration
    for (var trial = 1; trial <=ntrials; trial++){
            ...
            for (var i = 1; i <= repeat; i++) {
                ...
                Wavegen.Channel1.Mode.text = "Custom";
                Wavegen.Channel1.Custom.Type.text = "ramp" + i;
                pulseAndCapture(save_user,1,trial);
                wait(parseInt(delay));
...
if (Tool.question("start post air")) {
    Wavegen.Channel1.Mode.text = "Custom";
    Wavegen.Channel1.Custom.Amplitude.value = 3.4;
    Wavegen.Channel1.Custom.Offset.value = 1.6;
    Wavegen.Channel1.Custom.Frequency.value = 5;
    const questions = ["pre", "sample", "post"]
            ...
            for (var i = 1; i <= repeat; i++) {
                ...
                Wavegen.Channel1.Mode.text = "Custom";
                Wavegen.Channel1.Custom.Type.text = "ramp" + i;
                pulseAndCapture(save_user,2,trial);
                wait(parseInt(delay));
