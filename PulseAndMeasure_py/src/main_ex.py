...
def play_waveform(analogOut, analogIn, waveform, waveform_duration, wait_duration):
    ...
    (amplitude_min, amplitude_max) = analogOut.nodeAmplitudeInfo(1, node)
    ...
    analogOut.repeatSet(channel, 5)

    analogIn.reset()
    analogIn.channelEnableSet(channel, True)
    ...
    analogIn.configure(True, True)
    analogOut.configure(False, True)

    print(analogIn.statusData(channel, 4096))
...
def main():
    ...
    try:
        ...
        with openDwfDevice(dwf, serial_number_filter=args.serial_number_filter,
            ...
            play_waveform(device.analogOut, device.analogIn, waveform, args.waveform_duration, args.wait_duration)
