...
class trigger_source:
    ...
    wavegen1 = constants.trigsrcAnalogOut1
...
def trigger(device_data, enable, source=trigger_source.none, channel=1, timeout=0, edge_rising=True, level=0):
    """
        ...
        parameters: - device data
                    ...
                    - trigger source - possible: none, analog, digital, external[1-4], wavegen 1
...
def single_arm(device_data, channel):
    """
        Arm the scope for a single shot aqcuisition

        parameters: - device data
                    - the selected channel

    """
    # set device to single buffer acquisition
    dwf.FDwfAnalogInAcquisitionModeSet(device_data.handle, constants.acqmodeSingle1)
...
def single_read(device_data, channel):
    """
        Read the scope buffer after a single aqcuisition is captured
