...
def generate(device_data, channel, function, offset, frequency=1e03, amplitude=1, symmetry=50, wait=0, run_time=0, repeat=0, data=[]):
    ...
    # set wavegen to not ignore repeat/run cycle settings
    if dwf.FDwfAnalogOutRepeatTriggerSet(device_data.handle, channel, ctypes.c_bool(True)) == 0:
        check_error()
    ...
    # start
    if dwf.FDwfAnalogOutConfigure(device_data.handle, channel, ctypes.c_int(1)) == 0:
        check_error()
    return
