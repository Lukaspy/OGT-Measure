...
if (save_user) {
    //var baseFile = Tool.getSaveFile("Filename/Directoty", "/home/lukas/Documents/CampbellResearch/ENVIR-OGT Dilutions")//Tool.getText("Filename/directory")
    var baseFile = Tool.getSaveFile("Filename/Directoty", "C:/Users/LukasC/Documents/Campbell/Dilution Experiment PT2/Set4")//Tool.getText("Filename/directory")

    savepath = baseFile.split("/").slice(0, -1).join("/")
    nameArray = baseFile.split("/")
    nameArray = nameArray[nameArray.length-1].split("_")
    deviceNum = nameArray[0]
}
var repeat =  4//Tool.getText("Repeat times?")
var delay = 1//Tool.getText("Delay? (s)")
print(baseFile)
...
function pulseAndCapture(save,q) {
    Scope1.single();
    wait(.1);
    Wavegen.run();
    Scope1.wait();
    if (save) {
        if (q == 0) { //Pre
            Scope1.Export(savepath + "/" + deviceNum+"_air_pre_"+i.toString()+".csv", szView = "", fComments = true, fHeader = true, fLabel = true, fHeaderAsComment = false, szNotes = "")
        }
        if (q == 1) { //Sample
            Scope1.Export(baseFile+"_"+i.toString()+".csv", szView = "", fComments = true, fHeader = true, fLabel = true, fHeaderAsComment = false, szNotes = "")
        }
        if (q == 2) { //Post
            Scope1.Export(savepath + "/" + deviceNum+"_air_post_"+i.toString()+".csv", szView = "", fComments = true, fHeader = true, fLabel = true, fHeaderAsComment = false, szNotes = "")
        }
...
function tripleSquare(t1, t2, t3) {
    generated = []
    ...
    //Generate first ramp
    for (var i = 0; i < t1; i++) {
        generated[Math.round(i)] = 1//(i*rate)+.25;
    }
    ...
    var j = 0;
    for (var i = t1+t2+t3; i <= (2*t1)+t2+t3; i++) {
        generated[Math.round(i)] = 1//(j*rate)+.25;
        j++
    }
...
if (true) {//Tool.question("3 ramps")) {
    Wavegen.Channel1.Mode.text = "Custom";
    Wavegen.Channel1.Custom.Amplitude.value = 3.4;
    Wavegen.Channel1.Custom.Offset.value = 1.6;
    Wavegen.Channel1.Custom.Frequency.value = 5;
    const questions = ["pre", "sample", "post"]
// Check if user wants space bt ramps to increase each iteration
    for (var q = 0; q <3; q++) {
    if (Tool.question("Start " + questions[q] + "?")) {//Tool.question("Increase ramp spacing each time?")) {
        for (var i = 1; i <= repeat; i++) {
            var arbPulse = [];
            ...
            switch(i) {
                case 1:
                    arbPulse = tripleSquare(ramp_t1_user, ramp_t2_user, .2);
                    break;
                case 2:
                    arbPulse = tripleSquare(ramp_t1_user, ramp_t2_user, ramp_t3_user);
                    break;
                case 3:
                    arbPulse = tripleSquare(ramp_t1_user, ramp_t2_user, ramp_t3_user*5);
                    break;
                case 4:
                    arbPulse = tripleSquare(ramp_t1_user, ramp_t2_user, ramp_t3_user*15);
                    break;

            }

            Wavegen.Custom.set("square" + i, arbPulse);
            Wavegen.Channel1.Mode.text = "Custom";
            Wavegen.Channel1.Custom.Type.text = "square" + i;
            pulseAndCapture(save_user,q);
            wait(parseInt(delay));
        ...
        //Wavegen.Custom.set("ramp1", arbPulse);
        Wavegen.Channel1.Custom.Type.text = "ramp1";
        for (var i = 1; i <= repeat; i++) {
            pulseAndCapture(save_user);
           wait(parseInt(delay));
